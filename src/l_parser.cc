		...
		while (true)
		{
			...
			if(c == ':'){
			    if(rules.find(alphabet_char) == rules.end()){
                    double percentage = parser.readDouble();
                    std::pair<std::string, double> p = {rule,percentage};
                    std::vector<std::pair<std::string, double>> v = {p};
                    rules[alphabet_char] = v;
                } else {
			        double percentage = parser.readDouble();
                    std::pair<std::string, double> p = {rule,percentage};
                    rules[alphabet_char].push_back(p);
			    }
            } else {
                std::pair<std::string, double> p = {rule,1.0};
                std::vector<std::pair<std::string, double>> v = {p};
                rules[alphabet_char] = v;
			}
			parser.skip_comments_and_whitespace();
...
std::string LParser::LSystem::get_replacement(char c) const
{
	...
	for(std::pair<std::string, double> p: replacementrules.at(c)){
	    if(randDouble<=p.second){
	        return std::string(p.first);
	    } else {
	        randDouble = randDouble - p.second;
	    }
	}
}
